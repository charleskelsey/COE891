...
public class MoneyBagTest extends TestCase {
    ...
    public void testMoneyEqualsSpecific() {
        Money m1 = new Money(10, "USD");
        Money m2 = new Money(10, "USD");
        Money m3 = new Money(15, "USD");
        Money m4 = new Money(10, "CHF");
        Money zero1 = new Money(0, "USD");
        Money zero2 = new Money(0, "CHF"); // Note: isZero ignores currency

        assertTrue(m1.equals(m2));
        assertFalse(m1.equals(m3));
        assertFalse(m1.equals(m4));
        assertFalse(m1.equals(zero1));
        assertFalse(m1.equals("string")); // Test against different type
        assertFalse(m1.equals(null)); // Already implicitly tested by !f12CHF.equals(null)

        assertTrue(zero1.isZero());
        assertTrue(zero2.isZero());
        assertTrue(zero1.equals(zero2)); // isZero check dominates
        assertTrue(zero1.equals(new Money(0, "USD")));
        assertFalse(zero1.equals(m1));
    }

    public void testMoneyBagEqualsSpecific() {
        MoneyBag mb_10_USD_5_CHF = (MoneyBag) MoneyBag.create(new Money(10, "USD"), new Money(5, "CHF"));
        MoneyBag mb_10_USD_5_CHF_alt = (MoneyBag) MoneyBag.create(new Money(5, "CHF"), new Money(10, "USD")); // Order shouldn't matter
        MoneyBag mb_10_USD_6_CHF = (MoneyBag) MoneyBag.create(new Money(10, "USD"), new Money(6, "CHF"));
        MoneyBag mb_10_USD = new MoneyBag(); // Create empty then add
        mb_10_USD.appendMoney(new Money(10, "USD")); // Will simplify to Money, need careful testing
        Money single_10_USD = new Money(10,"USD");

        MoneyBag zeroBag1 = new MoneyBag();
        MoneyBag zeroBag2 = (MoneyBag) MoneyBag.create(new Money(0,"USD"), new Money(0,"CHF")); // Should be zero
        Money zeroMoney = new Money(0, "EUR");


        assertTrue(mb_10_USD_5_CHF.equals(mb_10_USD_5_CHF_alt));
        assertFalse(mb_10_USD_5_CHF.equals(mb_10_USD_6_CHF));
        assertFalse(mb_10_USD_5_CHF.equals(single_10_USD));
        assertFalse(mb_10_USD_5_CHF.equals("string"));

        assertTrue(zeroBag1.isZero());
        assertTrue(zeroBag2.isZero());
        assertTrue(zeroMoney.isZero());
        assertTrue(zeroBag1.equals(zeroBag2));
        assertTrue(zeroBag1.equals(zeroMoney)); // Zero bag equals zero money
        assertTrue(zeroMoney.equals(zeroBag1)); // And vice versa
        assertFalse(zeroBag1.equals(mb_10_USD_5_CHF));

    }

    // Add a test for toString if not already asserted
    public void testMoneyBagToString() {
         // fMB1 was {[12 CHF][7 USD]}
         IMoney mb1 = MoneyBag.create(new Money(12, "CHF"), new Money(7, "USD"));
         // Note: Order inside the bag isn't guaranteed, depends on Vector add order.
         // Let's assume appendMoney adds deterministically for the test
         // If not, we need a more robust check (e.g., contains both parts)
         // A safer check might be:
         String actual = mb1.toString(); // e.g., "{[12 CHF][7 USD]}" or "{[7 USD][12 CHF]}"
         assertTrue(actual.contains("[12 CHF]"));
         assertTrue(actual.contains("[7 USD]"));
